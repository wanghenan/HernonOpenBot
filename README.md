# 🦞 HernonOpenBot — Personal AI Assistant

> 快速上手（5分钟） → [详细文档](https://docs.openclaw.ai) · [架构解析](#architecture-quick-view) · [贡献指南](CONTRIBUTING.md)
...
HernonOpenBot/
├── 📄 README.md                    # 项目主入口（本文档）
├── 📄 AGENTS.md                    # 开发者指南（开发规范、命令速查）
├── 📄 CONTRIBUTING.md              # 贡献指南
├── 📄 CHANGELOG.md                 # 版本变更日志
├── 📄 SECURITY.md                  # 安全指南
│
├── 📁 docs/                        # Mintlify 在线文档（详细使用指南）
│   ├── index.md                    # 文档首页
│   ├── start/                      # 入门指南
│   ├── install/                    # 安装部署
│   ├── channels/                   # 消息通道配置
...
| 模块 | 职责 | 关键文件 |
|------|------|----------|
| `cli/` | CLI 入口、命令注册 | `program.ts`, `progress.ts` |
| `commands/` | 180+ 命令实现 | `send.ts`, `agent.ts`, `status.ts` |
| `config/` | 配置 schema 定义 | `zod-schema.*.ts` |
| `gateway/` | WS 控制平面 | `server.ts`, `ws-*.ts` |
| `agents/` | Pi Agent 运行时 | `index.ts`, `runtime.ts` |
| `channels/` | 消息路由/抽象 | `index.ts`, `routing/` |
| `plugins/` | 插件系统 | `manager.ts`, `config-schema.ts` |
| `hooks/` | 消息/事件拦截 | `index.ts`, `bundled/` |
| `providers/` | LLM 提供商 | `openai.ts`, `anthropic.ts` |
| `infra/` | 工具函数 | `deps.ts`, `logging.ts` |

### 文档贡献指南

| 场景 | 文档位置 | 说明 |
|------|----------|------|
| 新增配置项 | `src/config/zod-schema.*.ts` | 添加 schema 定义 |
| 新增命令 | `src/commands/` | 实现 Command 接口 |
| 新增通道 | `src/channels/` | 实现 Channel 接口 |
| 新增工具 | `src/tools/` | 实现 Tool 接口 |
| 更新使用文档 | `docs/` | Mintlify 格式 |
| 更新开发规范 | `AGENTS.md` | 开发者约定 |

### 架构决策记录（ADR）

重要架构决策记录在 `docs/reference/adr/` 目录下：
- ADR-001: 验证库选择（Zod vs Ajv）
- ADR-002: 插件架构设计
- ADR-003: 通道抽象设计
...
```bash
# 在终端直接对话
openclaw agent --message "Hello, I'm your new assistant!"

# 或配置消息通道后，在 WhatsApp/Telegram/Discord 等聊天
```
...
| 功能 | 命令 |
|------|------|
| 对话助手 | `openclaw agent --message "<内容>"` |
| 发送消息 | `openclaw message send --to <目标> --message "<内容>"` |
| 查看状态 | `openclaw status` |
| 配置通道 | `openclaw channels login` |
...
```bash
# 克隆项目
git clone https://github.com/wanghenan/HernonOpenBot.git
cd HernonOpenBot
...
- **多通道消息**: WhatsApp, Telegram, Slack, Discord, Signal, iMessage, Microsoft Teams, Matrix, Zalo, WebChat
- **AI 助手**: 基于 Pi Agent，支持工具调用和流式响应
- **跨平台**: macOS, iOS, Android, Linux, Windows (WSL2)
- **本地控制**: Gateway WebSocket 控制平面
- **技能系统**: 54+ 内置技能，支持运行时安装
- **实时 Canvas**: 视觉工作空间，A2UI 渲染
...
- [通道设置](https://docs.openclaw.ai/channels)
...
#### 修复项
- ✅ **插件配置验证增强** (`src/plugins/schema-validator.ts`)
  - 实现了完整的 JSON Schema 到 Zod 转换器
  - 支持 object/array/string/number/boolean/enum 等类型验证
...
- ✅ 构建成功 (`pnpm build`)
- ✅ 测试套件通过 (`pnpm test` - 192 tests passed)

#### 新增扩展
- ✅ **WeChat 微信通道插件** (`extensions/wechat/`)
  - 支持微信公众平台 (Official Account)
  - 支持 Webhook 和轮询两种模式
  - 支持文本和图片消息
  - 提供完整的中文配置文档
...
<p align="center">
  <a href="https://github.com/wanghenan/HernonOpenBot">
    <img src="https://img.shields.io/github/stars/wanghenan/HernonOpenBot?style=for-the-badge" alt="Stars">
  </a>
  <a href="https://github.com/wanghenan/HernonOpenBot/issues">
    <img src="https://img.shields.io/github/issues/wanghenan/HernonOpenBot?style=for-the-badge" alt="Issues">
  </a>
  <a href="https://github.com/wanghenan/HernonOpenBot/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/wanghenan/HernonOpenBot?style=for-the-badge" alt="License">
  </a>
</p>
